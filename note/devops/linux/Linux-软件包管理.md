...
RPM包安装的服务，可以使用系统服务管理命令`service`管理。比如：`service nginx start` service能找得到nginx服务，其实是去系统默认的安装目录当中寻找nginx相关的启动命令将nginx拿出来执行。因此service没发安装源码包安装的软件，因为安装的位置都不同。而源码包安装的服务只能用绝对路径的方式启动。因为二者安装的目录不同，所以带来了使用服务的方式不同。

### 源码包安装

1. `./configure`进行软件的配置和检查。它可以定义需要的功能选项，检测系统环境是否符合安装要求，最后把定义好的功能和检测系统环境的信息都写入Makefile文件当中，用于后续的编辑。

2. `make`进行编译（这两步不会真正的往指定安装目录写入数据，如果出错在这两步骤，只需要`make clean`清除编译的文件就好）

3. `make install`正式安装（到这里就是正式往安装目录写入数据了）
...
实际生产上，只会选择一种包安装，而不会源码包安装一次，RPM包安装一次。（推荐源码包，因为所有东西都安装在一个目录，没有垃圾文件，删除也干净！）对外提供访问的服务还是使用源码包比较好。
